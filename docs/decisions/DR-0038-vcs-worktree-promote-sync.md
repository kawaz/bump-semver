...
- branch-aware な justfile push gate が `bump-semver vcs is on-default-branch` ベースで書ける (= 下記 Adoption pattern 参照、`vcs is worktree` は別質問用に保持)。利用側 (= 各 kawaz リポの justfile) からの dogfooding が即可能。
...
## Adoption pattern (justfile push gate)

v0.40.0 land 直後の dogfood で **kawaz の jj 運用が `vcs is worktree` ベースの gate と合わない** 盲点を発見した:

- kawaz の git bare + jj workspace 方式では default workspace は repo root 直下、`main/` は `jj workspace add main` で作った **secondary workspace**
- `vcs is worktree` は実装通りに `main` workspace でも true を返す (= 設計正しい、`main` は secondary)
- そのため `vcs is worktree` ベースの gate は **main workspace からの push を block** する誤検出に

正しい gate predicate は **`vcs is on-default-branch` の反転** (= 「default branch 上にいなければ警告」)。これは「現 bookmark/branch が push 対象の default か?」という、実際に問うべき質問に対応する:
...
`vcs is worktree` predicate 自体は git linked worktree シナリオで有用なので削除しない。両 predicate を持つ意義は明確に分離 = `is worktree` は「**場所** (linked worktree かどうか)」、`is on-default-branch` は「**bookmark/branch** (default かどうか)」。push gate には後者が適している。
...
- v0.40.0 で land、v0.40.1 で `vcs promote` bareVerb 短絡 bug fix + adoption pattern 確定 (= 上記)
- 後続フォローアップ (本 DR の範囲外):
  - claude-rules-personal の `worktree-workflow-runbook-template.md` 起票 — `just push` ゲートを各 kawaz リポに展開する標準テンプレ (`check-on-default-branch` パターンを採用)
