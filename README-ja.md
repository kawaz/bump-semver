...
git/jj を抽象化した小さなヘルパー群 ([DR-0020](./docs/decisions/DR-0020-vcs-subcommands.md))。PR-1 で `vcs get` (read-only) が land、PR-2 で `vcs is` (述語) が加わる。残りの verb (`vcs diff` / `vcs commit` / `vcs push` / `vcs tag`) は段階的に追加予定。動機: Taskfile / justfile で git と jj を毎回手書き分岐する板挟みの解消。`bump-semver` は既に `vcs:` で VCS read を吸収しているので、その自然な拡張として `vcs` サブコマンド群を同居させる。
...
終了コード (詳細は後述): `0` 成功 / 述語が真; `1` 述語が偽 (`vcs is`); `2` usage エラー; `3` VCS 実行エラー (= 「リポではない」を含む); `4` 曖昧 (`5` は将来の `vcs push` non-ff 用に予約)。
