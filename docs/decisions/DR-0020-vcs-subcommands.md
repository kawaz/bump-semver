...
- PR-1〜PR-6 + PR-4.1 + PR-5.1 land 済 (`vcs get` / `vcs is` / `vcs diff` (+ `-s`/`-q`) / `vcs commit` (+ amend symmetry refinements) / `vcs fetch` + `vcs push` (+ hint simplification / jj export retry / passthrough) / `vcs tag push` + `vcs tag delete` (atomic create+push / 冪等 delete、`--allow-move` opt-in、別 rev integrity 違反は exit 4))。残りは PR-7 (移行+docs)
