...
- **hard link は切断される**: rename は新 inode で path を置換するため、対象 file への
  hard link は旧内容の旧 inode を指したまま残る (旧実装の in-place truncate+write は
  inode を共有したまま更新していた)。アトミック書き込みの本質的帰結として許容する
- **read-only ディレクトリ内の writable file は更新不可になる**: temp file の作成に
  ディレクトリ書き込み権限が必要なため (旧実装は既存 file を直接 open できた)。
  permission denied (exit 2) で何も書かずに終了する。同じく本質的トレードオフとして許容
- all-or-nothing 保証はテストでは synthetic handler (Replace を強制失敗させる fake) で
  構造的に検証している。実 format の「Inspect 成功後に Replace だけ失敗」を end-to-end
  で再現するテストは持たない (保証は writeBumpedFiles の二相構造そのものに由来する)
