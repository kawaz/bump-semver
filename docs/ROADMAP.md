...
get / bump 系の出力を構造化 JSON で返す (`name` / `version` / `semver` / `major` / `minor` / `patch` / `pre` / `pre_id` / `pre_rest` / `build_metadata` / `build_id` / `build_rest`)。CI で jq と組み合わせて使う用途。`compare` は対象外。スキーマ確定済、実装は v0.6.0 候補。詳細: [docs/issue/2026-05-10-json-output.md](./issue/2026-05-10-json-output.md)。
