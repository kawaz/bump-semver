...
Go ソースは `src/` 配下に隔離し、リポジトリ直下にはメタ情報 (README / docs / Taskfile.pkl / VERSION / go.mod 等) のみを置く。`go.mod` 自体はリポジトリ直下のままで、import path / module path は `github.com/kawaz/bump-semver` から変わらない。ビルドは `go build ./src`。
...
pkf run bump-version [patch|minor|major]
  ↓
ensure-clean → test → build → VERSION 書き換え → jj describe + new → pkf run push
  ↓
