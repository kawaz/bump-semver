...
**`vcs get latest-tag [--include-prerelease] [--repository REPO] [--json]`** および **`vcs get latest-release [--include-prerelease] [--repository REPO] [--json]`** (v0.32.0+、[DR-0032](./docs/decisions/DR-0032-vcs-get-latest-by-source-verb.md)) — SemVer 最大の tag / GitHub Release を出力する。source 軸 (tag list か Release オブジェクトか) は verb 名に畳まれ、各 verb は単一の責務を持つ。1-liner ergonomic な等価形 `vcs:latest-tag([REPO])` / `vcs:latest-release([REPO])` も入力 record として使用可能 ([vcs: 入力](#vcs-入力) 参照)。
...
| `--json` | bare SemVer | `get --json` と同一の 12-field schema (`{"name":..., "version":..., "semver":..., "major":..., ...}`)。`.version` は raw tag 文字列を保持 (旧 `--raw` 相当の情報を内包)、`.semver` は canonical bare 形、`.name` は `pkf-tasks@0.0.13` 形式の monorepo prefix を抽出 |
...
# 入力 record 経路 (v0.32.0 で DR-0032 により復活した 1-liner):
...
> v0.32.0 ([DR-0032](./docs/decisions/DR-0032-vcs-get-latest-by-source-verb.md)) で latest-tag / latest-release を上記の symmetric な 2 経路に再整理: 1-liner ergonomic 向けの scalar-returning 入力 record + 詳細 option 向けの [`vcs get latest-{tag,release}`](#vcs-サブコマンド) subcommand。v0.29.0 の `vcs tag latest [--source <tag|release>]` 形を再 supersede (source 軸を verb 名に畳む)。
