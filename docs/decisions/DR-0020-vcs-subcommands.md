...
- PR-1〜PR-6 + PR-4.1 + PR-5.1 + PR-5.2 + PR-2.2 land 済 (`vcs get` / `vcs is` (canonical empty 判定 = PR-2.2) / `vcs diff` (+ `-s`/`-q`) / `vcs commit` (+ amend symmetry refinements) / `vcs fetch` + `vcs push` (+ hint simplification / jj export retry / passthrough / `--jj-bookmark-auto-advance` opt-in による jj bookmark 自動移動) / `vcs tag push` + `vcs tag delete` (atomic create+push / 冪等 delete、`--allow-move` opt-in、別 rev integrity 違反は exit 4))。残りは PR-7 (移行+docs)
...
- `gitBackend.Push`: 防衛的 reject (`exitCodeVCSExec` + 「please file a
  bug」wording)。ここに到達する経路は dispatcher で塞いであるので **本来
  unreachable**、将来 dispatcher refactor で塞ぎ穴が空いた場合に backend が
  silent ignore せず塞ぐ belt-and-suspenders
...
| ON | git | (irrelevant) | (irrelevant) | exit 2 + 「jj-specific; this repo uses git」hint |
