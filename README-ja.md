...
**`vcs tag latest [--source <tag|release>] [--include-prerelease] [--repository REPO] [--raw | --json]`** (v0.29.0+、DR-0020 PR-Tag-Latest) — SemVer 最大の tag を出力する。削除された `vcs:latest-tag([REPO])` 入力の代替。tag / GitHub Release を列挙し、SemVer 2.0.0 でパース不可なものを落として最大を返す。

| フラグ | デフォルト | 意味 |
|---|---|---|
| `--source tag` (default) | tag | git/jj の tag 一覧。gh 不要。`--repository` を指定すると `git ls-remote --tags <url>` 経由 (こちらも gh 不要) |
| `--source release` | — | `gh release list` 経由 (GitHub Release オブジェクト) — draft は常にスキップ、prerelease は `--include-prerelease` 無しならスキップ。**`gh` CLI が必須** (未インストールなら exit 3 + インストールヒント) |
| `--repository REPO` | cwd VCS | 外部対象: `owner/repo` (GitHub 短縮、`https://github.com/...` に展開) or HTTPS/SSH フル URL。`--source` と直交 |
| `--include-prerelease` | 除外 | pre-release tag (`v1.2.3-rc.1` 等) を含める。旧 `vcs:latest-tag()` は常に prerelease を含めていたため、`--include-prerelease` がバイト等価な移行ターゲット |
| `--raw` | bare SemVer | prefix を保持した raw tag 文字列を出力 (`v1.2.3` / `release-1.2.3` / `pkf-tasks@0.0.13`)。`--json` と排他 |
| `--json` | bare SemVer | `{"tag": "v1.2.3", "version": "1.2.3", "commit": "...", "date": "..."}`。`commit` / `date` は best-effort — source が提供する場合のみ埋まる (release 経路は `date` を埋める)、空の場合あり (追加 subprocess は走らせない) |

```bash
bump-semver vcs tag latest                          # cwd: 1.2.3 (bare SemVer)
bump-semver vcs tag latest --raw                    # cwd: v1.2.3 (prefix 保持)
bump-semver vcs tag latest --include-prerelease     # v1.2.3-rc.1 も対象
bump-semver vcs tag latest --repository kawaz/pkf-tasks
                                                    # remote (git ls-remote --tags)
bump-semver vcs tag latest --source release         # cwd GitHub Releases (gh 必須)

# 旧 `vcs:latest-tag()` からの移行 (v0.28.x 以前):
...
終了コード: `0` 成功; `2` usage (`--source` 値不正、`--raw` + `--json` 同時指定、余分な positional); `3` VCS / gh subprocess エラー OR `--source release` で `gh` 未インストール。
...
> **最新 tag の取得**: v0.29.0 で `vcs:latest-tag([REPO])` 入力モードは削除。代替は [`bump-semver vcs tag latest`](#vcs-subcommands) サブコマンド。capture-then-compare 形式: `LATEST=$(bump-semver vcs tag latest); bump-semver compare gt VERSION "$LATEST"`。DR-0020 PR-Tag-Latest 参照 (`--source release` / `--raw` / `--json` / `--repository` 拡張)。
...
LATEST=$(bump-semver vcs tag latest); bump-semver compare gt Cargo.toml "$LATEST"  # ready to release? (CI: bump 済か確認)
...
LATEST=$(bump-semver vcs tag latest --repository kawaz/pkf-tasks)  # owner/repo 短縮
bump-semver compare ge 0.0.13 "$LATEST"                            # 現 pin は最新追従済?
```
...
> 最新 tag 取得は v0.29.0 で `vcs:` 入力 schema から外れた (DR-0020 PR-Tag-Latest)。代替は [`bump-semver vcs tag latest`](#vcs-subcommands) サブコマンド — `--source tag` (default、gh 不要) / `--source release` (GitHub Releases、`gh` CLI 必要)、`--repository owner/repo` / `--raw` / `--json` / `--include-prerelease` の各 toggle 付き。歴史的経緯は DR-0019 の "Superseded" 注記参照。
...
CI スクリプトを VCS 中立にしたい場合は jj/git どちらでも通る形式 (`origin/main` (jj 側で `main@origin` に自動フォールバック) / commit hash) を推奨。「最新リリースタグ」は [`vcs tag latest`](#vcs-subcommands) サブコマンドを使う (jj/git 自動判定は `vcs:` 入力と同じ)。
