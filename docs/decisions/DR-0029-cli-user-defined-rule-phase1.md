...
| `--version-regex <PATTERN>` | regex で version 抽出 (text 必須・全文 regex、json/yaml/toml/xml では path 値への 2 段抽出として併用可。structured で regex 単独 = path なしは error) |
...
path / regex の使い分けは **format で決まる** (= format ごとに役割を分離。全文 regex を
structured format でも許すと text format と機能重複し、parse コストだけ払って path を
使わない不整合が生じるため):

| format | `--version-path` | `--version-regex` | 挙動 |
|---|---|---|---|
| `text` | 不可 (error) | **必須** | 全文に regex を適用、capture group 1 = version |
| `json`/`yaml`/`toml`/`xml` | **必須** | なし | path で取得した値 = version |
| `json`/`yaml`/`toml`/`xml` | **必須** | あり | path で取得した container 値に regex を適用、capture group 1 = version (2 段抽出) |

- structured format で `--version-regex` のみ (path なし) を指定すると error
  (= 「全文 regex したいなら `--format text`」へ誘導)
- `--name-path` + `--name-regex` も対称 (= structured は name-path 主、name-regex 併用可)
...
**書き戻しアルゴリズム** (= format / 組み合わせ別):

1. **`--format text` (regex のみ)**: ファイル全文に regex を当て、capture group 1 の
   byte range だけを新 version 文字列で置換 (= DR-0012 と同じ挙動、line-anchored 推奨)
2. **structured + path のみ**: path で取った scalar string を version として読み、書き戻し
   時は path 値全体を新 version 文字列で完全置換
3. **structured + path + regex** (2 段): path で container 値を取得、regex group 1 を
   新 version に差し替えた **新 container 文字列** を作り、それを path 値全体として書き戻す
   (= 既存 format Replace が値の byte range だけを書き換えるので、container の version
   以外の bytes と周辺ドキュメントは byte-identical に保たれる)。**複数 `--version-path`
   がある場合は path ごとに container を個別取得・個別差し替えする** (= path[0] の
   container を他 path に流用しない。codex review 反映: `.name="myapp v1.2.3"` と
   `.label="release-1.2.3"` のように version 同値でも container が異なるケースで後者を
   破壊しないため)
