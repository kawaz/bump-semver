...
pending_entered:
discarded_entered:
resolved_entered: 2026-06-29T04:53:20+09:00
discard_reason:
pending_reason:
...
## 訂正 (2026-06-27, from grapheme.mbt session acad9f77...)

前回追記で「kawaz 各リポで `-qq` が定着して多用」と書いたが、これは **観測ベース誤り**。

grapheme.mbt セッションが canonical 側 (bump-semver, claude-cmux-msg, claude-push-guard, claude-nandakke, claude-gh-monitor, kuu.mbt, timespec.mbt, claude-plugin-reference) の justfile + .github/workflows 全件 audit したところ:

- canonical 側は **元から意図単位で `-qq` / `--quiet` を使い分け済み**
- `-qq` の出現はすべて exit code 用途 (= 自前 error 出すので bump-semver の stderr 抑制) で意図的に正しい
- grapheme.mbt 7 箇所 `-qq` の撒き散らしは **独自コピペミス** (canonical 模倣ではなく初期の盲信コピペ)

## 修正された問題定義

「kawaz 慣習として `-qq` が定着しているが文書化されていない」 → ❌ 取り消し

「`-qq` は cobra 内部仕様 (= `-q` の重ね指定で `--quiet-all` に格上げ) として動くが help / docs に明示されていない。利用者が誤コピペで撒いた際、意図単位の見直しに必要な情報が help にない」 → ✅ こちらが本来の問題定義

## 結論 (変わらず)

**提案 a (help 文 1 行追記) で十分**。grapheme.mbt 提案の help 文案そのまま採用候補。理由:

- canonical 側は元から正しいので、新規利用者の **誤コピペ防止** + **意図単位レビューの判断材料** が目的
- formal alias 化 (提案 c) は不要、cobra counting flag のままで OK
- README quiet section (提案 b) は a があれば不要

## アクションアイテム

- src/help.go or cobra help text の `-q / --quiet` 説明に「repeat (-qq) for --quiet-all behavior」を追記
- 全 sub-command で `-q` を持つ箇所すべてに同じ文言で一貫性確保
- v0.42.0 同梱
