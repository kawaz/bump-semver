...
- Prerequisite: DR-0030 (format=regex 廃止 + format=text + version-regex 統合)。本 DR の `--format` enum (Phase 1 は 4 値: text/json/yaml/toml) は DR-0030 後の internal 整理を前提とする。実装順は **DR-0030 → DR-0029**。
...
| `--version-path <DOTPATH>` | 構造化 path で version 抽出 (json/yaml/toml)、text では error |
| `--version-regex <PATTERN>` | regex で version 抽出 (text 必須、json/yaml/toml で path 値への 2 段抽出 or 全文 regex として併用可) |
| `--name-path <DOTPATH>` | name 抽出 (構造化、optional) |
| `--name-regex <PATTERN>` | name 抽出 (regex、optional) |

xml は Phase 1 では `--format` enum 対象外 (= 後述「Out of scope」)。
...
- **dead block**: invocation 内に「どの SOURCE にもマッチしなかった `--define-rule`
  ...
  されると debug 困難)
- **dead global**: 全 SOURCE がいずれかの `--define-rule` ブロックにマッチ → グローバル
  rule 系 flag は使われない (= dead code)。これは **warning** で stderr hint
  ("note: global --format/--version-* flags are unused because every SOURCE is covered
  by --define-rule blocks")。`--no-hint` / `-q` / `-qq` で抑制可能
...
のみ。`"key with dot"` の quoted key は Phase 2+ 検討。yaml / toml も同じ dot-path 構文を
共有 (= parse 結果を tree として扱う)。
...
- Phase 1 の確定事項は Phase 2 config file (= 別 DR) へ素直に拡張可能だが、config
  schema は CLI block の direct serialization ではなく internal rule object 寄り
  (= `version_paths[]` / `match_mode` / `rewrite_mode`) を採用予定

## Out of scope (Phase 2+ で扱う)

- **config file** (= `.bump-semver.toml` 等、Phase 2 別 DR)。Phase 1 CLI block は「CLI
  体験として最適化された形」、Phase 2 config は「internal rule object に近い form」
  を採用する想定 (= `version_paths: []` / `match_mode: "exactly-one"|"first"` /
  `rewrite_mode: "scalar"|"multi-sync"`)
- **`--format xml`**: XML は JSON/YAML/TOML と木の semantics が違う (= 要素繰り返し /
  属性 / テキストノード / 名前空間 / root anchoring) ため、共通 dot-path で扱う契約に
  無理がある。Phase 1 では XML 自作 file は対象外、Phase 2+ で別 path language
  (= XPath subset / slash-rooted path / 既存 builtin `xml-element` 仕様の踏襲) を別 DR
  で設計してから露出
- **builtin 無効化オプション** (= `--no-builtin-rules` / `--disable-builtin <PATTERN>` /
  config の `disable_builtin` リスト)。CLI rule の override で事実上の防御が効くため
  Phase 1 では入れない。後発 builtin との衝突が実際に起き、上書きでは不十分な事例が
  観測されてから Phase 2+ で別 DR
